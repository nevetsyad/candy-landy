// Simple working game to test basic functionality
...
document.addEventListener('keydown', (e) => {
    ...
    // Start game on SPACE, ENTER, or UP arrow
...
function drawStartScreen() {
    ...
    ctx.fillText('Collect all the candies and reach the platforms!', canvas.width / 2, 380);
    
    // Character preview
    ctx.fillStyle = '#ff69b4';
    ctx.fillRect(canvas.width / 2 - 20, 450, 40, 60);
    ctx.fillStyle = '#8b4513';
    ctx.fillRect(canvas.width / 2 - 25, 440, 50, 10); // Hair
    ctx.fillStyle = '#333';
    ctx.beginPath();
    ctx.arc(canvas.width / 2, 430, 15, 0, Math.PI * 2); // Head
    ctx.fill();
}
...
function drawGame() {
    ...
    // Player details
    ctx.fillStyle = '#8b4513';
    ctx.fillRect(player.x - 5, player.y - 10, 50, 10); // Hair
    ctx.fillStyle = '#333';
    ctx.beginPath();
    ctx.arc(player.x + 20, player.y - 15, 15, 0, Math.PI * 2); // Head
    ctx.fill();
